    ...
    const checkProvider = () =>
        new Promise((resolve, reject) => {
            if (typeof window.ethereum === "undefined")
                reject({ message: MESSAGES.WARNINGS.INSTALL_METAMASK });
    ...
    const getTokenBalance = async (tokenAddress) => {
        ...
        try {
            ...
            const [bal, decimal, symbol] = await Promise.all([
                ...
                contract.methods.decimals(userAddress).call(),
    ...
    const transferToken = async (recepAddress, amount) => {
        try {
            if (Object.keys(token).length === 0)
                throw { message: MESSAGES.WARNINGS.SUBMIT_TOKEN };
